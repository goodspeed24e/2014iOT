...
path_of_configuration_file_for_this_script = '/usr/local/id/streaming_pb_downloader/current/configs/downloader.ini'
...
try:
	...
	instek_logger.error('When we read log mail configurations. Unexpected error: %s (%s)', sys.exc_info()[0], sys.exc_info()[1])
	pass
...
class SingleInstanceObject:
	...
	def __init__(self, filename):
		...
		try:
			...
			instek_logger.info('When chmod file %s. Unexpected error: %s (%s)', filename, sys.exc_info()[0], sys.exc_info()[1])
...
try:
	...
	instek_logger.error('When open lock file %s. Unexpected error: %s (%s)', lockfile, sys.exc_info()[0], sys.exc_info()[1])
	sys.exit (1)
...
try:
	...
	instek_logger.error('When we read alarm configurations. Unexpected error: %s (%s)', sys.exc_info()[0], sys.exc_info()[1])
	# FixMe: We need to trigger alarm??
	pass
...
def trigger_alarm(event_subject_arg='DOWNLOADER-127.0.0.1_0_00000000_000000_00000000_000000-ERROR'):
	...
	try:
		...
		instek_logger.critical('When we trigger event: event subject = %s. Unexpected error: %s (%s)', event_subject_arg, sys.exc_info()[0], sys.exc_info()[1])
...
try:
	...
	temp_args = '/sbin/ifconfig eth0 | grep \"inet addr\" | awk -F: \'{ print $2 }\' | awk \'{ print $1 }\''
	child_p = subprocess.Popen(args=temp_args, stdout=subprocess.PIPE, shell=True)
	remote_ip = str(child_p.communicate()[0]).strip()
	#remote_ip = "127.0.0.1"
	...
	download_folder = mount_point + '/' + remote_ip + '_download'
	...
	last_check_event_datetime_filename = remote_ip + '_last_check_event_datetime.txt'
	...
	instek_logger.error('When we read configurations. Unexpected error: %s (%s)', sys.exc_info()[0], sys.exc_info()[1])
	trigger_alarm()
	sys.exit (1)
...
def check_mount_point(mount_point_arg):
	...
	try:
		...
		temp_args = "mount -l -t nfs,nfs4 | grep -w " + mount_point_arg
		child_p = subprocess.Popen(args=temp_args, stdout=subprocess.PIPE, shell=True)
		check_mount_out = str(child_p.communicate()[0]).strip()
		...
		instek_logger.error('When we check NFS mount point %s. Unexpected error: %s (%s)', mount_point_arg, sys.exc_info()[0], sys.exc_info()[1])
...
def check_free_space(mount_point_arg, limited_min_space_arg):
	...
	try:
		...
		temp_args = "df -P | grep -w " + mount_point_arg + ' | awk \'{print $4}\''
		child_p = subprocess.Popen(args=temp_args, stdout=subprocess.PIPE, shell=True)
		free_space_out = str(child_p.communicate()[0]).strip()
		...
		instek_logger.error('When we check the available space of mount point %s. Unexpected error: %s (%s)', mount_point_arg, sys.exc_info()[0], sys.exc_info()[1])
	...
	while(overdue_date >= min_overdue_date):
		...
		if os.path.exists(overdue_day_folder):
			try:
				...
				instek_logger.error('When remove folder %s. Unexpected error: %s (%s)', overdue_day_folder, sys.exc_info()[0], sys.exc_info()[1])
		...
		if (overdue_date.day == 1) or (overdue_date == min_overdue_date):
			...
			if os.path.exists(overdue_month_folder):
				try:
					...
					instek_logger.error('When remove folder %s. Unexpected error: %s (%s)', overdue_month_folder, sys.exc_info()[0], sys.exc_info()[1])
	...
	while(delete_date_lower_bound <= delete_date_upper_bound):
		...
		if os.path.exists(delete_day_folder):
			try:
				...
				instek_logger.error('When remove folder %s. Unexpected error: %s (%s)', delete_day_folder, sys.exc_info()[0], sys.exc_info()[1])
		...
		if (delete_date_lower_bound.day == calendar.monthrange(delete_date_lower_bound.year, delete_date_lower_bound.month)[1]):
			...
			if os.path.exists(delete_month_folder):
				try:
					...
					instek_logger.error('When remove folder %s. Unexpected error: %s (%s)', delete_month_folder, sys.exc_info()[0], sys.exc_info()[1])
...
def generate_sql_condition_of_event_name(filename_of_event_name_file_arg):
	...
	try:
		...
		instek_logger.error('When read event names from file %s. Unexpected error: %s (%s)', filename_of_event_name_file_arg, sys.exc_info()[0], sys.exc_info()[1])
...
def generate_sql_condition_of_event_time(last_check_event_datetime_filename_arg, seconds_ago_event_arg):
	...
	try:
		...
		instek_logger.error('When read event time from file %s. Unexpected error: %s (%s)', last_check_event_datetime_filename_arg, sys.exc_info()[0], sys.exc_info()[1])
	...
	try:
		...
		instek_logger.error('When write last check event time to file %s. Unexpected error: %s (%s)', last_check_event_datetime_filename_arg, sys.exc_info()[0], sys.exc_info()[1])
		return "";
...
def create_sub_directory(dir_name_arg):
	...
	try:
		...
		instek_logger.error('When create folder %s. Unexpected error: %s (%s)', dir_name_arg, sys.exc_info()[0], sys.exc_info()[1])
...
def launch_downloader_to_run(download_cmd_arg):
	...
	try:
		...
		instek_logger.error('When launch downloader. Unexpected error: %s (%s)', sys.exc_info()[0], sys.exc_info()[1])
	finally:
		if ret_value != 0:
			if ret_value == -10009:
...
if create_sub_directory(download_folder) == False:
	instek_logger.error('Can not create download folder: %s.', download_folder)
	trigger_alarm()
	sys.exit (1)

# Remove overdue data.
remove_overdue_data(reserved_days, download_folder, min_date_for_check)
...
temp_last_check_event_datetime_filename = download_folder + "/" + last_check_event_datetime_filename
...
try:
	...
	for i in range(num_of_rows):
		...
		temp_event_name = record["event_name"]

		# Calculate the epoch time of start and end time
		utc_start_time_sec = calendar.timegm(temp_start_time.timetuple())
		utc_end_time_sec = calendar.timegm(temp_end_time.timetuple())

		# Calculate the local time of start and end time
		local_start_datetime = datetime.datetime.fromtimestamp(utc_start_time_sec)
		local_end_datetime = datetime.datetime.fromtimestamp(utc_end_time_sec)
		
		# Construct the output sub-folder. (FixMe: Use start time or end time to create output folder??)
		year_month_subfolder = download_folder + "/" + local_start_datetime.strftime("%Y-%m")
		iavf_output_path = year_month_subfolder + "/" + local_start_datetime.strftime("%d")
		...
		# Get the camera name from XML file
 		temp_args = 'grep \"<Name>\" /usr/local/id/hdvr/current/conf/xml/mdpersist-010-cm-' + str(temp_channel_id).zfill(2) + '-CameraSetupBasic.xml | cut -d\">\" -f 2 | cut -d\"<\" -f 1'
		child_p = subprocess.Popen(args=temp_args, stdout=subprocess.PIPE, shell=True)
		temp_camera_name = str(child_p.communicate()[0]).strip()
		instek_logger.info('CAM-%02d Name = %s', temp_channel_id, temp_camera_name)

		# Construct the output filename
		temp_filename = temp_event_name + "_IP_" + remote_ip + "_CAM" + str(temp_channel_id).zfill(2) + "_" + temp_camera_name + "_Time_" + local_start_datetime.strftime("%Y%m%d-%H%M%S") + "_" + local_end_datetime.strftime("%Y%m%d-%H%M%S")

		# Construct the download command
		download_cmd = downloader + " " + remote_ip + " " + str(playback_port) + " " + str(temp_channel_id) + " " + str(utc_start_time_sec) + " " + str(utc_end_time_sec) + " \"" + iavf_output_path + "\" \"" + idb_output_path + "\" \"" + temp_filename + "\" " + str(split_file_size) + " " + str(time_zone) + " \"" + remote_username + "\" \"" + remote_password + "\""

		instek_logger.info('%s', download_cmd)

		# Start downloading.
		if launch_downloader_to_run(download_cmd) == False:
			temp_event_subject = 'DOWNLOADER-' + remote_ip + '_' + str(temp_channel_id) + '_' + local_start_datetime.strftime("%Y%m%d_%H%M%S") + '_'+ local_end_datetime.strftime("%Y%m%d_%H%M%S") + '-ERROR'
			...
			if (temp_free_space_ret[0] == True) and (temp_free_space_ret[1] == False):
				...
				remove_old_data_for_available_space(reserved_days_for_not_enough_space, download_folder, min_date_for_check, mount_point, min_available_mount_point_size)
	...
	instek_logger.error('When we use MySQL. Unexpected error: %s (%s)', sys.exc_info()[0], sys.exc_info()[1])
	trigger_alarm()
	sys.exit (1)
finally:
	if cursor:
...
try:
	...
	instek_logger.error('When we try to remove temporary folder. Unexpected error: %s (%s)', sys.exc_info()[0], sys.exc_info()[1])
	trigger_alarm()
	sys.exit (1)
