...
// sensor type
static EVENT_TYPE g_mageticResult = EVENT_TYPE_UNKNOWN;
...
void rf2rpi(void)
{
    ...
    while (radio.available() && !done ) 
    {
         ...
         if (done)
         {
            ...
            if (atoi(token[ELEMENT_TYPE_EVENT1]) == (int)EVENT_TYPE_MAGETIC)
            {
              g_mageticResult = EVENT_TYPE_UNKNOWN;  
              Serial.println("RESET!!! -- MAGETIC");
            }
         }
         delay(16);
    } // end of while 
    ...
    pkg.event1_type   = (int)g_mageticResult;
    pkg.return1_value = (int)g_mageticResult;
...
void setup_sensor()
{
  ...
  pinMode(MAGETIC_PIN,INPUT);
}

void magetic_sensor()
{
  int val = digitalRead(MAGETIC_PIN);
  if(val == HIGH)
  {
    Serial.println("[ALARM] - MAGETIC!!!");
    digitalWrite(BUZZER_PIN,HIGH);   delay(100);
    digitalWrite(BUZZER_PIN,LOW);    delay(100);
    
    g_mageticResult = EVENT_TYPE_MAGETIC;
  }
}
