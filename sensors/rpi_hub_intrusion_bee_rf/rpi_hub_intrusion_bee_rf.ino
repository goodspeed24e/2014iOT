...
// sensor type
static EVENT_TYPE g_intrusionResult = EVENT_TYPE_UNKNOWN;
...
void rf2rpi(void)
{
    ...
    while (radio.available() && !done ) 
    {
         ...
         if (done)
         {
            ...
            if (atoi(token[ELEMENT_TYPE_EVENT1]) == (int)EVENT_TYPE_INTRUSION)
            {
              g_intrusionResult = EVENT_TYPE_UNKNOWN;  
              Serial.println("RESET!!! -- INTRUSION");
            }
         }
         delay(16);
    } // end of while 
    ...
    pkg.event1_type   = (int)g_intrusionResult;
    pkg.return1_value = (int)g_intrusionResult;
...
void intrusion_sensor()
{
  int val = digitalRead(INTRUSION_PIN);
  if(val == HIGH)
  {
    Serial.println("[ALARM] - INTRUSION!!!");
    digitalWrite(BUZZER_PIN,HIGH);   delay(100);
    digitalWrite(BUZZER_PIN,LOW);    delay(100);
    
    g_intrusionResult = EVENT_TYPE_INTRUSION;
  }
}
